...
class plugin_signin_signin_shortcodes extends e_shortcode
{
	...
	private $usernamePlaceholder = LAN_SIGNIN_USERNAME;
	...
	function __construct()
	{
		...
		{
			$this->usernamePlaceholder = LAN_SIGNIN_EMAIL;
		}

		if ($pref['allowEmailLogin'] == 2)
		{
			$this->usernamePlaceholder = LAN_SIGNIN_USEREMAIL;
		}
	...
	function sc_signin_rememberme($parm = '')
	{
		...
		{
			return "<input type='checkbox' name='autologin' id='autologin' value='1' checked='checked' />" . ($parm ? $parm : "" . LAN_SIGNIN_REMEMBER );
		}
	...
	{
		// '<li><a href="'.e_ADMIN_ABS.'"><span class="fa fa-cogs"></span> '.LAN_SIGNIN_ADMIN.'</a></li>';
		if (ADMIN == true)
		{
			return  e_ADMIN_ABS; //  . 'admin.php' : '<a class="signin-sc admin" id="signin-sc-admin" href="' . e_ADMIN_ABS . 'admin.php">' . LAN_SIGNIN_ADMIN . '</a>';
		}
